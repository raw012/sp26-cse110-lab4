...
4. values added: 20
5. Error occurs because the variable "result" is not defined inside the else statmenet. As the result variable is defined by "let", which is limited inside the if block, so there will be an error in accessing the "result" outside the if block.
